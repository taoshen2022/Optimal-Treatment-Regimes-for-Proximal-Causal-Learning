...
def ITR_experiment(s_num, rep_num):
    ...
    for i in range(len(rew)):
        if A_tz[i] == A_tw[i]:
            ...
            qw = (Y * ((A == 1) * q1t_re[0] + (A == -1) * q0t_re[0]) * (A == 1) * (A_tw[i] == 1) + inter) +  (Y * ((A == 1) * q1t_re[0] + (A == -1) * q0t_re[0]) * (A == -1) * (A_tw[i] == -1) + inter)

            band = 1.06 * 500**(-0.2) * np.std(X[:,0])
            x_ker = gaussian_kernel(X, X_t[i,:], band)
            if np.sum(hz * x_ker)/np.sum(x_ker) >= np.sum(qw * x_ker)/np.sum(x_ker):
                A_tzw[i,0] = A_tz[i,0]
            else:
                A_tzw[i,0] = A_tw[i,0]
